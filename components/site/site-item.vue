...
import { Check, FolderInput, FolderOutput, Pencil, Trash2 } from 'lucide-vue-next'
...
const availableFolders = computed(() => {
  return gridItemStore.allFolders.filter(folder => folder.id !== props.item.pid)
})
...
function openEdit() {
  components?.siteEdit.value?.open({
    ...props.item
  })
}
