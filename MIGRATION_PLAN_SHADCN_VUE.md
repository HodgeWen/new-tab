...
- 右键菜单：`components/ui/ContextMenu.vue`
...
- Button：统一所有按钮样式与交互态。
- Input：统一所有输入框样式。
- ContextMenu / DropdownMenu：替换 `components/ui/ContextMenu.vue` 与编辑工具条的“移入分组”菜单。
...
## 3. 技术准备与依赖策略
### 3.1 依赖引入（计划）
- shadcn-vue 及其依赖（Radix Vue、class-variance-authority、tailwind-merge 等）。
- 若保持 UnoCSS：确认 UnoCSS 对 Tailwind 类名兼容，必要时补充主题变量与语义化色值。
...
## 4. 迁移策略（分阶段、可回滚）
### 阶段 0：基线与风险控制
- 建立 UI 基线：记录当前关键页面与弹层的截图/行为清单。
- 约定回滚策略：按组件维度提交，确保任意时刻可退回上一个稳定版本。

### 阶段 1：基础组件接入
- 初始化 shadcn-vue 目录结构（优先使用 `components/ui` 作为承载）。
...
- UnoCSS 与 shadcn 默认 Tailwind token 不一致：
  - 对策：补齐 CSS 变量与 token，必要时引入最小 Tailwind 配置或对 shadcn 组件进行 class 覆盖。
...
## 7. 建议的落地顺序（按此执行）
1) 基础组件接入（Button/Input/Select/Switch/Tabs）
2) SettingsPanel（最复杂、可验证基础组件）
3) AddSite/AddFolder/FolderModal（Dialog 统一）
4) ContextMenu + EditToolbar（菜单体系统一）
5) 卡片与细节收敛
